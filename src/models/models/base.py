...
import torch
import transformers
from src.models.encoders import str2encoder
from src.models.modules import LayerwiseAttention
...
class CometModel(ptl.LightningModule, metaclass=abc.ABCMeta):
    ...
    def __init__(
        ...
        # TODO switch back if we don't use rel pos
        self.encoder = str2encoder[self.hparams.encoder_model].from_pretrained(
            self.hparams.pretrained_model, 
        )
        print("using model ", self.hparams.pretrained_model)
        """
        self.decoder = str2decoder[self.hparams.encoder_model].from_pretrained(
            self.hparams.pretrained_model
        )
        """
        self.epoch_nr = 0
    ...
    def get_sentence_embedding(
        self, input_ids: torch.Tensor, attention_mask: torch.Tensor, hyp_inps: torch.Tensor = None, hyp_mask: torch.Tensor = None, hyp_posid: torch.Tensor = None) -> torch.Tensor:
        ...
        """
        if self.caching:
            return self.retrieve_sentence_embedding(input_ids, attention_mask, hyp_inps, hyp_mask, hyp_posid=hyp_posid)
        else:
            return self.compute_sentence_embedding(input_ids, attention_mask, hyp_inps, hyp_mask, hyp_posid=hyp_posid)

    @tensor_lru_cache(maxsize=CACHE_SIZE)
    def retrieve_sentence_embedding(
        self, input_ids: torch.Tensor, attention_mask: torch.Tensor, hyp_inps: torch.Tensor, hyp_mask: torch.Tensor, hyp_posid: torch.Tensor
    ) -> torch.Tensor:
        """Wrapper for `get_sentence_embedding` function that caches results."""
        return self.compute_sentence_embedding(input_ids, attention_mask, hyp_inps, hyp_mask, hyp_posid)

    def compute_sentence_embedding(
        self, input_ids: torch.Tensor, attention_mask: torch.Tensor, hyp_inps: torch.Tensor, hyp_mask: torch.Tensor, hyp_posid: torch.Tensor
    ) -> torch.Tensor:

        if hyp_inps is not None: # AKA we are using bart
            encoder_out = self.encoder(input_ids, attention_mask, decoder_input_ids=hyp_inps, decoder_attention_mask=hyp_mask, position_ids=hyp_posid)
        else: # using model where separate passes are needed
        ...
        # only need 1 pass with bart
        if hyp_inps is not None:
            return sentemb, encoder_out
        return sentemb
