...
def beam_search(
        ...
        **model_kwargs,
        ...
        if self.man_pref is not None:
            ...
            if self.man_pref.shape[0]==num_beams:
                input_ids = self.man_pref.to(self.device)
            elif self.man_pref.shape[0]<num_beams:
                input_ids = torch.zeros(num_beams, self.man_pref.shape[1]).long().to(self.device)
                # use starting number of prefixes
                input_ids[:self.man_pref.shape[0], :] = self.man_pref
            else:
        ...
        if self.man_pref is not None:
            if self.man_pref.shape[0]<num_beams:
                beam_scores[:, self.man_pref.shape[0]:] = -1e9
        else:
            beam_scores[:, 1:] = -1e9
