...
for epoch, batch in tqdm(enumerate(ppo_trainer.dataloader)):
    ...
    # expand to repeat stuff by certain amount
    allqt = []
    for i in range(int(1/keep_percent)):
        allqt.extend(query_tensors)
    query_tensors = allqt
    if first: 
    ...
    # find top N
    order = torch.argsort(torch.stack(rewards))
    topinds = order[-1*script_args.batch_size:]
